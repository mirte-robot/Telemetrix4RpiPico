...
#ifndef MPU9250_H
#define MPU9250_H
// Code for the MPU9250 copied and adapted from https://github.com/hideakitai/MPU9250 for the RP2040.
#include <stdint.h>
...
public:
    ...
    bool setup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), int bus = 0) {
        ...
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7)) {
            ...
            // // Serialprintln(" is not valid for MPU. Please check your I2C address.");
            return false;
        }
        mpu_i2c_addr = addr;
        setting = mpu_setting;
...
private:
    ...
    void collect_mag_data_to(float* m_bias, float* m_scale) {
        if (b_verbose)
            // Serialprintln("Mag Calibration: Wave device in a figure eight until done!");
        sleep_ms(4000);
        ...
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            ...
            if (MAG_MODE == 0x02) sleep_ms(135);  // at 8 Hz ODR, new mag data is available every 125 ms
            if (MAG_MODE == 0x06) sleep_ms(12);   // at 100 Hz ODR, new mag data is available every 10 ms
        }
