...
#ifndef MPU9250_H
#define MPU9250_H
// Code for the MPU9250 copied and adapted from
// https://github.com/hideakitai/MPU9250 for the RP2040.
...
class MPU9250_ {
  static constexpr uint8_t MPU9250_DEFAULT_ADDRESS{
      0x68};                                     // Device address when ADO = 0
  ...
  static constexpr uint8_t MAG_MODE{
      0x06}; // 0x02 for 8 Hz, 0x06 for 100 Hz continuous magnetometer data read
  ...
  float gyro_bias[3]{0., 0.,
                     0.}; // gyro calibration value in GYRO_FS_SEL: 250dps
  float mag_bias_factory[3]{0., 0., 0.};
  float mag_bias[3]{0., 0.,
                    0.}; // mag calibration value in MAG_OUTPUT_BITS: 16BITS
  ...
  float temperature{
      0.f}; // Stores the real internal chip temperature in degrees Celsius

  // Self Test
  float self_test_result[6]{
      0.f}; // holds results of gyro and accelerometer self test
  ...
  float lin_acc[3]{0.f, 0.f, 0.f}; // linear acceleration (acceleration with
                                   // gravity component subtracted)
...
public:
  ...
  void sleep(bool b) {
    uint8_t c = read_byte(mpu_i2c_addr,
                          PWR_MGMT_1); // read the value, change sleep bit to
                                       // match b, write byte back to register
    if (b) {
  ...
  bool update() {
    ...
    // So to adopt to the general Aircraft coordinate system (Right-Hand,
    // X-Forward, Z-Down), we need to feed (ax, -ay, -az, gx, -gy, -gz, my, -mx,
    // mz) but we pass (-ax, ay, az, gx, -gy, -gz, my, -mx, mz) because gravity
    // is by convention positive down, we need to ivnert the accel data

    // get quaternion based on aircraft coordinate (Right-Hand, X-Forward,
    // Z-Down) acc[mg], gyro[deg/s], mag [mG] gyro will be convert from [deg/s]
    // to [rad/s] inside of this function quat_filter.update(-a[0], a[1], a[2],
    // g[0] * DEG_TO_RAD, -g[1] * DEG_TO_RAD, -g[2] * DEG_TO_RAD, m[1], -m[0],
    // m[2], q);
    ...
    if (!b_ahrs) {
      ...
      temperature = ((float)temperature_count) / 333.87 +
...
private:
  void initMPU9250() {
    ...
    // reset device
    write_byte(mpu_i2c_addr, PWR_MGMT_1,
    ...
    // GYRO_DLPF_CFG = bits 2:0 = 011; this limits the sample rate to 1000 Hz
    // for both With the MPU9250, it is possible to get gyro sample rates of 32
    // kHz (!), 8 kHz, or 1 kHz
    ...
    write_byte(
        mpu_i2c_addr, SMPLRT_DIV,
        sample_rate); // Use a 200 Hz rate; a rate consistent with the filter
    ...
    uint8_t c = read_byte(
        mpu_i2c_addr, GYRO_CONFIG); // get current GYRO_CONFIG register value
    c = c & ~0xE0;                  // Clear self-test bits [7:5]
    c = c & ~0x03;                  // Clear Fchoice bits [1:0]
    c = c & ~0x18;                  // Clear GYRO_FS_SEL bits [4:3]
    c = c | (uint8_t(setting.gyro_fs_sel)
             << 3); // Set full scale range for the gyro
    c = c | (uint8_t(~setting.gyro_fchoice) & 0x03); // Set Fchoice for the gyro
    write_byte(mpu_i2c_addr, GYRO_CONFIG,
               c); // Write new GYRO_CONFIG value to register

    // Set accelerometer full-scale range configuration
    c = read_byte(mpu_i2c_addr,
                  ACCEL_CONFIG); // get current ACCEL_CONFIG register value
    c = c & ~0xE0;               // Clear self-test bits [7:5]
    c = c & ~0x18;               // Clear ACCEL_FS_SEL bits [4:3]
    c = c | (uint8_t(setting.accel_fs_sel)
             << 3); // Set full scale range for the accelerometer
    write_byte(mpu_i2c_addr, ACCEL_CONFIG,
    ...
    c = c | (uint8_t(setting.accel_dlpf_cfg) &
             0x07); // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz
    write_byte(mpu_i2c_addr, ACCEL_CONFIG2,
    ...
    write_byte(mpu_i2c_addr, INT_ENABLE,
  ...
  void initAK8963() {
    ...
    read_bytes(AK8963_ADDRESS, AK8963_ASAX, 3,
               &raw_data[0]); // Read the x-, y-, and z-axis calibration values
    mag_bias_factory[0] =
        (float)(raw_data[0] - 128) / 256. +
    ...
    // register, and enable continuous mode data acquisition MAG_MODE (bits
    // [3:0]), 0010 for 8 Hz and 0110 for 100 Hz sample rates
    write_byte(AK8963_ADDRESS, AK8963_CNTL,
               (uint8_t)setting.mag_output_bits << 4 |
                   MAG_MODE); // Set magnetometer data resolution and sample ODR
    sleep_ms(10);

    if (b_verbose) {
      // Serialprintln("Mag Factory Calibration Values: ");
      // Serialprint("X-Axis sensitivity offset value ");
      // Serialprintln(mag_bias_factory[0], 2);
      // Serialprint("Y-Axis sensitivity offset value ");
      // Serialprintln(mag_bias_factory[1], 2);
      // Serialprint("Z-Axis sensitivity offset value ");
      // Serialprintln(mag_bias_factory[2], 2);
    }
  }
...
public:
  ...
  void update_accel_gyro() {
    int16_t raw_acc_gyro_data[7]; // used to read all 14 bytes at once from the
                                  // MPU9250 accel/gyro
    read_accel_gyro(raw_acc_gyro_data); // INT cleared on any read

    // Now we'll calculate the accleration value into actual g's
    a[0] =
        (float)raw_acc_gyro_data[0] *
        acc_resolution; // get actual g value, this depends on scale being set
    ...
    g[0] = (float)raw_acc_gyro_data[4] *
           gyro_resolution; // get actual gyro value, this depends on scale
                            // being set
...
private:
  void read_accel_gyro(int16_t *destination) {
    ...
    read_bytes(mpu_i2c_addr, ACCEL_XOUT_H, 14,
               &raw_data[0]); // Read the 14 raw data registers into data array
    destination[0] =
        ((int16_t)raw_data[0] << 8) |
        (int16_t)raw_data[1]; // Turn the MSB and LSB into a signed 16-bit value
...
public:
  void update_mag() {
    int16_t mag_count[3] = {
        0, 0, 0}; // Stores the 16-bit signed magnetometer sensor output

    // Read the x/y/z adc values
    if (read_mag(mag_count)) {
      // Calculate the magnetometer values in milliGauss
      // Include factory calibration per data sheet and user environmental
      // corrections mag_bias is calcurated in 16BITS
      float bias_to_current_bits =
          mag_resolution / get_mag_resolution(MAG_OUTPUT_BITS::M16BITS);
      m[0] = (float)(mag_count[0] * mag_resolution * mag_bias_factory[0] -
                     mag_bias[0] * bias_to_current_bits) *
             mag_scale[0]; // get actual magnetometer value, this depends on
                           // scale being set
      m[1] = (float)(mag_count[1] * mag_resolution * mag_bias_factory[1] -
                     mag_bias[1] * bias_to_current_bits) *
             mag_scale[1];
      m[2] = (float)(mag_count[2] * mag_resolution * mag_bias_factory[2] -
                     mag_bias[2] * bias_to_current_bits) *
             mag_scale[2];
    }
  }
...
private:
  bool read_mag(int16_t *destination) {
    ...
    if (st1 & 0x01) {      // wait for magnetometer data ready bit to be set
      uint8_t raw_data[7]; // x/y/z gyro register data, ST2 register stored
                           // here, must read ST2 at end of data acquisition
      read_bytes(AK8963_ADDRESS, AK8963_XOUT_L, 7,
                 &raw_data[0]); // Read the six raw data and ST2 registers
                                // sequentially into data array
      if (MAG_MODE == 0x02 || MAG_MODE == 0x04 ||
          MAG_MODE == 0x06) {  // continuous or external trigger read mode
        if ((st1 & 0x02) != 0) // check if data is not skipped
          return false; // this should be after data reading to clear DRDY
                        // register
      }
      ...
      if (!(c & 0x08)) { // Check if magnetic sensor overflow set, if not then
                         // report data
        destination[0] =
            ((int16_t)raw_data[1] << 8) |
            raw_data[0]; // Turn the MSB and LSB into a signed 16-bit value
        destination[1] = ((int16_t)raw_data[3] << 8) |
                         raw_data[2]; // Data stored as little Endian
  ...
  int16_t read_temperature_data() {
    ...
    read_bytes(mpu_i2c_addr, TEMP_OUT_H, 2,
               &raw_data[0]); // Read the two raw data registers sequentially
                              // into data array
    return ((int16_t)raw_data[0] << 8) |
           raw_data[1]; // Turn the MSB and LSB into a 16-bit value
  }
  ...
  void set_acc_gyro_to_calibration() {
    ...
    write_byte(mpu_i2c_addr, USER_CTRL,
    ...
    write_byte(mpu_i2c_addr, FIFO_EN,
  ...
  void collect_acc_gyro_data_to(float *a_bias, float *g_bias) {
    ...
    read_bytes(mpu_i2c_addr, FIFO_COUNTH, 2,
    ...
    for (uint16_t ii = 0; ii < packet_count; ii++) {
      ...
      read_bytes(mpu_i2c_addr, FIFO_R_W, 12,
      ...
      a_bias[0] +=
          (float)accel_temp[0]; // Sum individual signed 16-bit biases to get
    ...
    a_bias[0] /=
        (float)packet_count; // Normalize sums to get average count biases
  ...
  void write_accel_offset() {
    ...
    int16_t acc_bias_reg[3] = {
        0, 0, 0}; // A place to hold the factory accelerometer trim biases
    read_bytes(mpu_i2c_addr, XA_OFFSET_H, 2,
               &read_data[0]); // Read factory accelerometer trim values
    ...
    int16_t mask_bit[3] = {
        1, 1,
        1}; // Define array to hold mask bit for each accelerometer bias axis
    for (int i = 0; i < 3; i++) {
      if (acc_bias_reg[i] % 2) {
        mask_bit[i] = 0;
      }
      acc_bias_reg[i] -=
          (int16_t)acc_bias[i] >> 3; // Subtract calculated averaged
                                     // accelerometer bias scaled to 2048 LSB/g
      if (mask_bit[i]) {
        acc_bias_reg[i] = acc_bias_reg[i] &
                          ~mask_bit[i]; // Preserve temperature compensation bit
      } else {
        acc_bias_reg[i] =
            acc_bias_reg[i] | 0x0001; // Preserve temperature compensation bit
      }
    }
  ...
  void write_gyro_offset() {
    ...
    gyro_offset_data[0] = (-(int16_t)gyro_bias[0] / 4 >> 8) &
                          0xFF; // Divide by 4 to get 32.9 LSB per deg/s to
                                // conform to expected bias input format
    gyro_offset_data[1] = (-(int16_t)gyro_bias[0] / 4) &
                          0xFF; // Biases are additive, so change sign on
  ...
  void calibrate_mag_impl() {
    ...
    if (b_verbose) {
      // Serialprintln("Mag Calibration done!");

      // Serialprintln("AK8963 mag biases (mG)");
      // Serialprint(mag_bias[0]);
      // Serialprint(", ");
      // Serialprint(mag_bias[1]);
      // Serialprint(", ");
      // Serialprint(mag_bias[2]);
      // Serialprintln();
      // Serialprintln("AK8963 mag scale (mG)");
      // Serialprint(mag_scale[0]);
      // Serialprint(", ");
      // Serialprint(mag_scale[1]);
      // Serialprint(", ");
      // Serialprint(mag_scale[2]);
      // Serialprintln();
    }
  ...
  void collect_mag_data_to(float *m_bias, float *m_scale) {
    ...
    if (b_verbose) {
      // Serialprintln("mag x min/max:");
      // Serialprintln(mag_min[0]);
      // Serialprintln(mag_max[0]);
      // Serialprintln("mag y min/max:");
      // Serialprintln(mag_min[1]);
      // Serialprintln(mag_max[1]);
      // Serialprintln("mag z min/max:");
      // Serialprintln(mag_min[2]);
      // Serialprintln(mag_max[2]);
    }
    ...
    m_bias[0] = (float)bias[0] * bias_resolution *
                mag_bias_factory[0]; // save mag biases in G for main program
    ...
    scale[1] = (float)(mag_max[1] - mag_min[1]) * mag_bias_factory[1] /
               2; // get average y axis max chord length in counts
    scale[2] = (float)(mag_max[2] - mag_min[2]) * mag_bias_factory[2] /
  ...
  // settings
  bool self_test_impl() // Should return percent deviation from factory trim
                        // values, +/- 14 or less deviation is a pass
  {
    ...
    write_byte(mpu_i2c_addr, MPU_CONFIG,
    ...
    for (int ii = 0; ii < 200;
      ...
      read_bytes(
          mpu_i2c_addr, ACCEL_XOUT_H, 6,
          &raw_data[0]); // Read the six raw data registers into data array
      aAvg[0] += (int16_t)(((int16_t)raw_data[0] << 8) |
                           raw_data[1]); // Turn the MSB and LSB into a signed
                                         // 16-bit value
      aAvg[1] += (int16_t)(((int16_t)raw_data[2] << 8) | raw_data[3]);
      aAvg[2] += (int16_t)(((int16_t)raw_data[4] << 8) | raw_data[5]);

      read_bytes(mpu_i2c_addr, GYRO_XOUT_H, 6,
                 &raw_data[0]); // Read the six raw data registers sequentially
                                // into data array
      gAvg[0] += (int16_t)(((int16_t)raw_data[0] << 8) |
                           raw_data[1]); // Turn the MSB and LSB into a signed
                                         // 16-bit value
    ...
    for (int ii = 0; ii < 200;
      ...
      read_bytes(
          mpu_i2c_addr, ACCEL_XOUT_H, 6,
          &raw_data[0]); // Read the six raw data registers into data array
      aSTAvg[0] += (int16_t)(((int16_t)raw_data[0] << 8) |
                             raw_data[1]); // Turn the MSB and LSB into a signed
                                           // 16-bit value
      ...
      read_bytes(mpu_i2c_addr, GYRO_XOUT_H, 6,
                 &raw_data[0]); // Read the six raw data registers sequentially
                                // into data array
      gSTAvg[0] += (int16_t)(((int16_t)raw_data[0] << 8) |
                             raw_data[1]); // Turn the MSB and LSB into a signed
                                           // 16-bit value
    ...
    self_test_data[0] = read_byte(
        mpu_i2c_addr, SELF_TEST_X_ACCEL); // X-axis accel self-test results
    self_test_data[1] = read_byte(
        mpu_i2c_addr, SELF_TEST_Y_ACCEL); // Y-axis accel self-test results
    self_test_data[2] = read_byte(
        mpu_i2c_addr, SELF_TEST_Z_ACCEL); // Z-axis accel self-test results
    self_test_data[3] = read_byte(
        mpu_i2c_addr, SELF_TEST_X_GYRO); // X-axis gyro self-test results
    self_test_data[4] = read_byte(
        mpu_i2c_addr, SELF_TEST_Y_GYRO); // Y-axis gyro self-test results
    self_test_data[5] = read_byte(
        mpu_i2c_addr, SELF_TEST_Z_GYRO); // Z-axis gyro self-test results

    // Retrieve factory self-test value from self-test code reads
    factoryTrim[0] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[0] -
                                 1.0))); // FT[Xa] factory trim calculation
    factoryTrim[1] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[1] -
                                 1.0))); // FT[Ya] factory trim calculation
    factoryTrim[2] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[2] -
                                 1.0))); // FT[Za] factory trim calculation
    factoryTrim[3] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[3] -
                                 1.0))); // FT[Xg] factory trim calculation
    factoryTrim[4] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[4] -
                                 1.0))); // FT[Yg] factory trim calculation
    factoryTrim[5] = (float)(2620 / 1 << FS) *
                     (pow(1.01, ((float)self_test_data[5] -
    ...
    for (int i = 0; i < 3; i++) {
      self_test_result[i] =
          100.0 * ((float)(aSTAvg[i] - aAvg[i])) / factoryTrim[i] -
    ...
    if (b_verbose) {
      // Serialprint("x-axis self test: acceleration trim within : ");
      // Serialprint(self_test_result[0], 1);
      // Serialprintln("% of factory value");
      // Serialprint("y-axis self test: acceleration trim within : ");
      // Serialprint(self_test_result[1], 1);
      // Serialprintln("% of factory value");
      // Serialprint("z-axis self test: acceleration trim within : ");
      // Serialprint(self_test_result[2], 1);
      // Serialprintln("% of factory value");
      // Serialprint("x-axis self test: gyration trim within : ");
      // Serialprint(self_test_result[3], 1);
      // Serialprintln("% of factory value");
      // Serialprint("y-axis self test: gyration trim within : ");
      // Serialprint(self_test_result[4], 1);
      // Serialprintln("% of factory value");
      // Serialprint("z-axis self test: gyration trim within : ");
      // Serialprint(self_test_result[5], 1);
      // Serialprintln("% of factory value");
    }
  ...
  void write_byte(uint8_t address, uint8_t subAddress, uint8_t data) {
    ...
    i2c_err_ = write_i2c(this->bus, address, {subAddress, data});
  ...
  uint8_t read_byte(uint8_t address, uint8_t subAddress) {
    // uint8_t data = 0;                         // `data` will store the
    // register data
    std::vector<uint8_t> data;
    data.resize(1, 0);
    read_i2c(this->bus, address, {subAddress}, 1, data);
    return data[0];

    // wire->beginTransmission(address);         // Initialize the Tx buffer
    // wire->write(subAddress);                  // Put slave register address
    // in Tx buffer i2c_err_ = wire->endTransmission(false);  // Send the Tx
    // buffer, but send a restart to keep connection alive if (i2c_err_)
    // print_i2c_error(); wire->requestFrom(address, (size_t)1);       // Read
    // one byte from slave register address if (wire->available()) data =
    // wire->read();  // Fill Rx buffer with result return data; // Return data
  ...
  void read_bytes(uint8_t address, uint8_t subAddress, uint8_t count,
    ...
    // in Tx buffer i2c_err_ = wire->endTransmission(false);  // Send the Tx
    // buffer, but send a restart to keep connection alive if (i2c_err_)
    // print_i2c_error(); uint8_t i = 0; wire->requestFrom(address, count);  //
    // Read bytes from slave register address while (wire->available()) {
    //     dest[i++] = wire->read();
