...
public:
    ...
    bool read(HiwonderCommands cmd, uint8_t *params, int param_len, uint8_t MYID)
    {
    ...
    {
        uint8_t params[1];
        return write(HiwonderCommands::MOVE_START, params, 1,
                     BROADCAST_ID);
    }
    /**
    ...
    {
        uint8_t params[1];
        write(HiwonderCommands::MOVE_STOP, params, 1,
              BROADCAST_ID);
    }
...
public:
    ...
    void setLimitsTicks(int32_t lower, int32_t upper)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            uint8_t params[] = {(uint8_t)lower, (uint8_t)(lower >> 8), (uint8_t)upper, (uint8_t)(upper >> 8)};
            commandOK = _bus->write(HiwonderCommands::ANGLE_LIMIT_WRITE, params,
                                    4, _id);
    ...
    void readLimits()
    {
        ...
        do
        {
            ...
            else
            {
                ...
                {
                    // Serial.println(
                    // "ERR MotorID:" + String(_id) + " Min set " + String(minCentDegrees) + " max = " + String(maxCentDegrees) + " Min ticks " + String(lowicks) + " max ticks = " + String(highticks));
                ...
                // else
                // Serial.println(
                // "MotorID:" + String(_id) + " Min set " + String(minCentDegrees) + " max = " + String(maxCentDegrees));
            }
    ...
    void move_time_and_wait_for_sync(int32_t angle, uint16_t time)
    {
        ...
        uint8_t params[] = {(uint8_t)angle, (uint8_t)(angle >> 8),
                            (uint8_t)time, (uint8_t)(time >> 8)};
        commandOK = _bus->write(HiwonderCommands::MOVE_TIME_WAIT_WRITE, params, 4,
                                _id);
    }
    ...
    {
        uint8_t params[] = {0};
        commandOK = _bus->write(HiwonderCommands::LOAD_OR_UNLOAD_WRITE, params, 1, _id);
    }
    /**
     * Command name: SERVO_LOAD_OR_UNLOAD_WRITE
     Command value: 31 Length: 4
    ...
    void motor_mode(int16_t speed)
    {
        ...
        commandOK = _bus->write(HiwonderCommands::OR_MOTOR_MODE_WRITE, params, 4,
                                _id);
        if (commandOK)
    ...
    {
        uint8_t params[1];
        _bus->write(HiwonderCommands::ANGLE_OFFSET_WRITE, params, 1,
                    _id);
    }
    // angle_adjust sets the position angle offset in centi-degrees (-3000..3000)
    void angle_offset_adjust(int16_t angle)
    {
        ...
        commandOK = _bus->write(HiwonderCommands::ANGLE_OFFSET_ADJUST, params, 1,
                                _id);
    }
    // angle_adjust sets the position angle offset in centi-degrees (-3000..3000)
    ...
    int32_t pos_read()
    {
        ...
        commandOK = true;
        lastKnownGoodPosition = ((int16_t)params[0] | ((int16_t)params[1] << 8)) * 24;
        return pos_read_cached() + staticOffset;
    }
    /**
