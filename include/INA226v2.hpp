 ...
 *
 * You'll find several example sketches which should enable you to use the
 *library.
 *
 * You are free to use it, change it or build on it. In case you like it, it
 *would be cool if you give it a star.
 *
 * If you find bugs, please inform me!
 *
 * Written by Wolfgang (Wolle) Ewald
 * https://wolles-elektronikkiste.de/en/ina226-current-and-power-sensor
 *(English) https://wolles-elektronikkiste.de/ina226 (German)
 *
 *
 ******************************************************************************/
...
typedef enum INA226_CURRENT_RANGE { MA_400, MA_800 } currentRange;
...
public:
  ...
  static constexpr uint8_t INA226_CURRENT_REG{
      0x04}; // Current flowing through Shunt
  ...
  static constexpr uint8_t INA226_MAN_ID_REG{
      0xFE}; // Contains Unique Manbufacturer Identification Number
  ...
  static constexpr uint16_t INA226_ALERT_POL{
      0x0002}; // Alert pin polarity - if set then active-high
  // Latch enable - if set then alert flag remains until mask/enable register is
  // read if not set then flag is cleared after next conversion within limits
  static constexpr uint16_t INA226_LATCH_EN{0x0001};

  // Constructors: if not passed, 0x40 / Wire will be set as address / wire
  // object
  INA226_WE(const int addr = 0x40, const int bus = 1)
      : _wire(bus), i2cAddress{addr} {}
  ...
  void setConversionTime(INA226_CONV_TIME shuntConvTime,
                         INA226_CONV_TIME busConvTime);
  ...
  bool overflow = false;
  bool convAlert = false;
  bool limitAlert = false;

protected:
  INA226_AVERAGES deviceAverages = AVERAGE_1;
  INA226_CONV_TIME deviceConvTime = CONV_TIME_1100;
  INA226_MEASURE_MODE deviceMeasureMode = CONTINUOUS;
  INA226_CURRENT_RANGE deviceCurrentRange = MA_800;
  INA226_ALERT_TYPE deviceAlertType = SHUNT_OVER;
  int _wire = 1;
  int i2cAddress = 0x40;
  uint16_t calVal = 0;
