...
#define INA226_BU S_VOLTAGE 0x02
...
//  Constructor
//
INA226::INA226(const uint8_t address, int i2c_port)
{
  _address = address;
...
{
  return write_i2c(this->_i2c_port, this->_address, {});
}
...
float INA226::getBusVoltage()
{
  ...
  return val * 0.00125; //  fixed 1.25 mV
}
...
int INA226::setMaxCurrentShunt(float maxCurrent, float shunt, bool normalize)
{
  ...
  float shuntVoltage = std::abs(maxCurrent * shunt);
...
uint16_t INA226::_readRegister(uint8_t reg)
{
  ...
  if (!read_i2c(this->_i2c_port, this->_address, {reg}, 2, out)) {
    return -1;
  }

  uint16_t value = ( (uint16_t)(out[0]) << 8) | out[1];
  // value <<= 8;
  // value |= out[1];
  return value;
}
...
{
  return write_i2c(this->_i2c_port, this->_address, {reg, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF)});
}
