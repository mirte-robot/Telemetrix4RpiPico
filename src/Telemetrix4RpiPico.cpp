...
GPS_Sensor::GPS_Sensor(uint8_t settings[SENSORS_MAX_SETTINGS_A])
{
  ...
  // gpio_set_function(tx, GPIO_FUNC_UART); // No need to send anything to the GPS receiver
...
void INA226_Sensor::readSensor()
{
  ...
  if (time_us_32() - last_scan <= 200'000) // update every 200ms, sensor has new data every 1.1ms*256
  {
    return;
  }
