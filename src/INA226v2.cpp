...
void INA226_WE::reset_INA226() { writeRegister(INA226_CONF_REG, INA226_RST); }
...
void INA226_WE::setConversionTime(INA226_CONV_TIME shuntConvTime,
...
// set resistor and current range independant. resistor value in ohm, current
// range in A
...
float INA226_WE::getCurrent_A() { return (getCurrent_mA() / 1000); }
...
void INA226_WE::startSingleMeasurement() {
  ...
  unsigned long convStart = time_us_32() / 1000;
  while (!convReady && ((time_us_32() / 1000 - convStart) < 2000)) {
    convReady = ((readRegister(INA226_MASK_EN_REG)) &
                 0x0008); // checks if sampling is completed
  }
}
...
uint8_t INA226_WE::getI2cErrorCode() { return i2cErrorCode; }
...
void INA226_WE::writeRegister(uint8_t reg, uint16_t val) {
  ...
  write_i2c(this->_wire, this->i2cAddress, {reg, hVal, lVal});
}
...
uint16_t INA226_WE::readRegister(uint8_t reg) {
  ...
  read_i2c(this->_wire, this->i2cAddress, {reg}, 2, out);
  regValue = (out[0] << 8) + out[1];
  return regValue;
}
