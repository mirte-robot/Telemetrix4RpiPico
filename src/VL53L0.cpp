...
#include "VL53L0.hpp"
//  Adapted by Arend-Jan van Hilten for the RP2040 from the https://github.com/pololu/vl53l0x-arduino library
...
VL53L0X::VL53L0X()
    : bus(0), address(ADDRESS_DEFAULT), io_timeout(0) // no timeout
      ,
      did_timeout(false)
{
}
...
{
    write_i2c(this->bus, this->address, {reg, value});
...
{
    write_i2c(this->bus, this->address, {reg, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF)});
...
{
    write_i2c(this->bus, this->address, {reg, (uint8_t)(value >> 24 & 0xFF), (uint8_t)(value >> 16 & 0xFF), (uint8_t)(value >> 8 & 0xFF), (uint8_t)(value & 0xFF)});
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
    ...
    data.resize(1, 0);
    read_i2c(this->bus, this->address, {reg}, 1, data);
    //   bus->beginTransmission(address);
    //   bus->write(reg);
    //   last_status = bus->endTransmission();
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    std::vector<uint8_t> data;
    data.resize(2, 0);
    read_i2c(this->bus, this->address, {reg}, 2, data);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    std::vector<uint8_t> data;
    data.resize(4, 0);
    read_i2c(this->bus, this->address, {reg}, 4, data);
    return data[0] << 24 | data[1] << 16 | data[2] << 24 | data[3];
    // uint32_t value;
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count)
{
    ...
    write_i2c(this->bus, this->address, data);
...
void VL53L0X::readMulti(uint8_t reg, uint8_t *dst, uint8_t count)
{
    std::vector<uint8_t> data;
    data.resize(count, 0);
    read_i2c(this->bus, this->address, {reg}, count, data);
