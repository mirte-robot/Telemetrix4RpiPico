...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value) {
  write_i2c(this->bus, this->address,
            {reg, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF)});
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value) {
  write_i2c(this->bus, this->address,
            {reg, (uint8_t)(value >> 24 & 0xFF), (uint8_t)(value >> 16 & 0xFF),
...
bool VL53L0X::setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks) {
  ...
  if (type == VcselPeriodPreRange) {
    ...
    writeReg(MSRC_CONFIG_TIMEOUT_MACROP, (new_msrc_timeout_mclks > 256)
                                             ? 255
...
// intermediate values
void VL53L0X::getSequenceStepTimeouts(SequenceStepEnables const *enables,
                                      SequenceStepTimeouts *timeouts) {
